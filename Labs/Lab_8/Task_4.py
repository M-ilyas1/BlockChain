...
def concatenate_and_hash(data1, data2):
    """Concatenate two data strings and return the SHA-256 hash of the result."""
...
def merkle_tree_hashes(data_blocks):
    ...
    while len(current_level) > 1:
        ...
        for i in range(0, len(current_level), 2):
            combined = concatenate_and_hash(current_level[i], current_level[i+1] if i+1 < len(current_level) else current_level[i])
